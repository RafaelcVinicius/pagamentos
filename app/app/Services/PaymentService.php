...
class PaymentService
{
    ...
    private function prepareDataStore($payment, $data): array
    {
        ...
        if (array_key_exists('charges_details', $payment)) {
            foreach ($payment['charges_details'] as $value) {
                $newData["fee"][] = array(
                    "type" => $value['name'],
                    "original_amount" => $value['amounts']['original'],
                    "refunded_amount" => $value['amounts']['refunded'],
                    "valor" => $value['amounts']['original'],
                );
            }
        }
        ...
        if ($payment['payment_type_id'] == 'credit_card') {
            $newData['card'] = array(
                ...
                'first_digits' => $payment['card']['first_six_digits'],
                'last_digits' => $payment['card']['last_four_digits'],
            );
        }
        ...
        if ($payment['payment_method_id'] == 'pix') {
            $newData['pix'] = array(
                ...
                'expires_on' =>  \Carbon\Carbon::parse($payment['date_of_expiration'])->format('Y-m-d H:i:s'),
            );
        }
